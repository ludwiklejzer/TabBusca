...
function loadScript() {
	...
	// create and load basic html elements
	createElement("form", { id: "search-form" }, "#search-container");
	createElement("div", { id: "search-icon" }, "#search-form");
	createElement(
		"input",
		{
			id: "search-input",
			type: "search",
			placeholder: "Sobre o que deseja buscar?",
			autofocus: true,
		},
		"#search-form"
	);
	createElement("ul", { id: "results" }, "#search-container");
	const form = document.querySelector("#search-form");
	const input = document.querySelector("#search-input");
	const list = document.querySelector("#results");
	const searchIcon = document.querySelector("#search-icon");
	const svgIcon = `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#ffffff" > <g id="SVGRepo_bgCarrier" stroke-width="0"></g> <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" ></g> <g id="SVGRepo_iconCarrier"> <path d="M15 15L21 21" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" ></path> <path d="M17 10C17 13.866 13.866 17 10 17C6.13401 17 3 13.866 3 10C3 6.13401 6.13401 3 10 3C13.866 3 17 6.13401 17 10Z" stroke="#ffffff" stroke-width="2" ></path> </g> </svg>`;
	searchIcon.innerHTML = svgIcon;
	...
	setCss("#search-form", { display: "flex", marginLeft: "10px" });
	setCss("#search-input", {
		outline: "none",
		border: "none",
		borderRadius: "5px",
	...
	setCss("#results", {
		...
		borderRadius: "12px",
		boxShadow:
			"rgb(27 31 36 / 12%) 0px 1px 3px, rgb(66 74 83 / 12%) 0px 8px 24px",
		width: "300px",
		transition: ".6s all",
		opacity: "0",
		zIndex: "-1",
	...
	function setCss(identifier, property) {
	...
	function setSpinner(state) {
		if (state === "on") {
			createElement("span", { id: "search-spinner" }, "#results");
			const spinner = document.querySelector("#search-spinner");

			setCss("#results", { minHeight: "100px" });
			setCss("#search-spinner", {
				width: "40px",
				height: "40px",
			...
			setCss("#search-spinner", { display: "none" });
			setCss("#results", { minHeight: "0px" });
		}
	}
	...
	function parseData(data) {
		...
		for (let i = 0; i < h3s.length - 1; i++) {
			const title = h3s[i].textContent;
			const regexOnlyPosts = /https*:\/\/www\.tabnews\.com\.br\/.+\/.+/; // get only posts
			let link = h3s[i].parentNode.href;

			if (proxy === "allOrigins") {
				link = h3s[i].parentNode.parentNode.parentNode.href;
				const regexFilterLink = /.+=(.+)&sa=/; // retrieve only the true link
				link = link.match(regexFilterLink)[1];
			}

			if (regexOnlyPosts.test(link)) {
		...
		if (list.children.length <= 1) {
			const li = document.createElement("li");
			const a = document.createElement("a");
			a.innerText = "Nenhum resultado encontrado";
			a.href = "";
			li.appendChild(a);
			list.appendChild(li);
		}
		...
		setCss("#results li", {
			listStyleType: "none",
			borderBottom: "1px solid #24292f14",
		});
		setCss("#results a", {
			textDecoration: "none",
			color: "#24292f",
	...
	function getData(userInput) {
		let input = removeDiacritics(userInput);
		const url = `https://www.google.com/search?q=inurl%3Atabnews.com.br+inurl%3A${input}&num=50`;
		let proxyUrl;
